...
class Server(Thread):
    def __init__(self, config):
        ...
        # ICMP "Listener" only need 1 thread
        th_icmp = threading.Thread(
            name="ICMP Thread",
            target=self.proxy_icmp_thread,
            args=(self.icmpSocket, config),
        )
        # th_icmp.setDaemon(True)
        th_icmp.start()
    ...
    def proxy_icmp_thread(self, icmpSocket, config):
        # Waiting for ICMP connection
        while True:
            data, addr = icmpSocket.recvfrom(config["ICMP_BUFFERSIZE"])
